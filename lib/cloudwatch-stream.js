...
function CloudWatchStream (options) {
  ...
  } else if (options.aws_creds_json) {
    AWS.config.loadFromPath(options.aws_creds_json);
  } else if (options.aws_creds_profile) {
    // rely on Amazon SDK pulling credentials from the ~/.aws/credentials; region still needs to be defined
    var iniFileCredentials = new AWS.SharedIniFileCredentials({ profile: options.aws_creds_profile });
    AWS.config.credentials = iniFileCredentials;
    cloudWatchOptions = {
      credentials: iniFileCredentials,
      region: options.aws_region,
    };
  } else {
    // rely on Amazon SDK pulling credentials from IAM Role when running on EC2; region still needs to be defined
    cloudWatchOptions = {
      region: options.aws_region,
    };
  }  
  ...
  this.cloudWatchLogs = new AWS.CloudWatchLogs(cloudWatchOptions);
